...
from ddcsschema.error import DecodeError, URIParseError
from ddcsschema.uri import parse_ddcs_uri

from ddcsumserver import deserialize
from ddcsumserver.storage import Storage
from ddcsumserver.processor import logger
from ddcsumserver.utils import int_to_hex, hex_to_int, print_log
...
class ClaimsStorage(Storage):
    ...
    def import_signed_claim_transaction(self, claim, claim_id, undo_info):
        ...
        try:
            ...
            parsed_uri = parse_ddcs_uri(claim.name)
            if decoded_claim.has_signature:
                cert_id = decoded_claim.certificate_id
            else:
                cert_id = None
        except Exception as e:
            logger.warn("decode error for ddcs://{}#{}".format(claim.name, claim_id))
            decoded_claim = None
            cert_id = None
