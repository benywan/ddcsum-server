...
from ddcsschema.error import DecodeError, URIParseError, CertificateError
from ddcsschema.uri import parse_ddcs_uri

from ddcsumserver.processor import print_log, logger
from ddcsumserver.utils import bc_address_to_hash_160, hex_to_int, int_to_hex, Hash
...
class Storage(object):
    ...
    # convert between ddcs addresses and 20 bytes keys used for storage.
    @staticmethod
    ...
    def listunspent(self, addr):
        key = self.address_to_key(addr)
        if key is None:
            raise BaseException('Invalid ddcscrd address', addr)
        out = []
